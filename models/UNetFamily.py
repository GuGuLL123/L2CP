...
class U_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1, pooling_type='max', prethick=False):
        ...
        self.prethick = prethick
        self.Conv1 = conv_block(ch_in=img_ch, ch_out=64, dropout_p=0.25)
        self.Conv2 = conv_block(ch_in=64, ch_out=128, dropout_p=0.25)
        self.Conv3 = conv_block(ch_in=128, ch_out=256, dropout_p=0.25)
        self.Conv4 = conv_block(ch_in=256, ch_out=512, dropout_p=0.25)
        self.Conv5 = conv_block(ch_in=512, ch_out=1024, dropout_p=0.25)
        ...
        if prethick:
            self.Convthick_1x1 = nn.Conv2d(64, 1, kernel_size=1, stride=1, padding=0)
    def forward(self, x):
        # encoding path
        x1 = self.Conv1(x)
        ...
        if self.prethick:
            thick = self.Convthick_1x1(d2)
            thick = F.sigmoid(thick)
        if self.prethick:
            return d1, thick
        else:
            return d1
...
# ==================================================================
class U_Net_Fusion(nn.Module):
    def __init__(self, img_ch=3, output_ch=1, pooling_type='max', prethick=False):
        super(U_Net_Fusion, self).__init__()
        if pooling_type == 'max':
            self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        elif pooling_type == 'avg':
            self.pool = nn.AvgPool2d(kernel_size=2, stride=2)
...
class Dense_Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x1
# =========================================================
class U_Net_Wave_V1(nn.Module):
    """ 使用wavelet下采样，使用原来Unet上采样"""
    def __init__(self, img_ch=3, output_ch=1,wavename = 'haar'):
        super(U_Net_Wave_V1, self).__init__()
    ...
    def forward(self, x):
        ...
        x2, LL_1, LH_1, HH_1, = self.downsampling_1(x1)
        x2 = self.Conv2(x2)

        x3, LL_2, LH_2, HH_2, = self.downsampling_2(x2)
        x3 = self.Conv3(x3)

        x4, LL_3, LH_3, HH_3, = self.downsampling_3(x3)
        x4 = self.Conv4(x4)

        x5, LL_4, LH_4, HH_4, = self.downsampling_4(x4)
        x5 = self.Conv5(x5)
...
class U_Net_Wave_V2(nn.Module):
    """ 使用DWT下采样，使用IDWT上采样"""
    def __init__(self, img_ch=3, output_ch=1,wavename = 'haar'):
        super(U_Net_Wave_V2, self).__init__()
        ...
        self.downsampling_3 = DWT_2D(wavename=wavename)
        self.downsampling_4 = DWT_2D(wavename=wavename)
        self.downsampling_5 = DWT_2D(wavename=wavename)
        self.upsampling_4 = IDWT_2D(wavename=wavename)
        self.upsampling_3 = IDWT_2D(wavename=wavename)
        self.upsampling_2 = IDWT_2D(wavename=wavename)
        self.upsampling_1 = IDWT_2D(wavename=wavename)
    ...
    def forward(self, x):
        ...
        d5 = self.upsampling_4(d5, LL_4, LH_4, HH_4)
        d5 = torch.add(x4, d5)

        d4 = self.Up_conv4(d5)

        d4 = self.upsampling_3(d4, LL_3, LH_3, HH_3)
        d4 = torch.add(x3, d4)
        d3 = self.Up_conv3(d4)

        d3 = self.upsampling_2(d3, LL_2, LH_2, HH_2)
        d3 = torch.add(x2, d3)
        d2 = self.Up_conv2(d3)

        d2 = self.upsampling_1(d2, LL_1, LH_1, HH_1)
        d2 = torch.add(x1, d2)
