...
def concat_result(ori_img,pred_res,gt,save_path_new):


    ori_img = np.transpose(ori_img,(1,2,0))
    pred_res = np.transpose(pred_res,(1,2,0))
    gt = np.transpose(gt,(1,2,0))
    cv2.imwrite(save_path_new + '_ori_img.png',ori_img[:,:,0] * 255)
    cv2.imwrite(save_path_new + '_pred_res.png',pred_res[:,:,0] * 255)
    cv2.imwrite(save_path_new + '_gt.png',gt[:,:,0] * 255)

    binary = deepcopy(pred_res)
    binary[binary>=0.5]=1
    binary[binary<0.5]=0

    cv2.imwrite(save_path_new + '_binary.png',binary[:,:,0] * 255)
