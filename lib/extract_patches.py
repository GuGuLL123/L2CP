...
from lib.pre_processing import rgb2gray
...
def get_data_test_overlap(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    ...
    test_imgs, _ = my_PreProc(test_imgs_original)
    test_masks = test_masks/255.
    test_FOVs = test_FOVs//255
    #extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs,add_test_masks = paint_border_overlap(test_imgs,test_masks, patch_height, patch_width, stride_height, stride_width)
    ...
    patches_imgs_test,patches_masks_test = extract_ordered_overlap(test_imgs,add_test_masks,patch_height,patch_width,stride_height,stride_width)
    ...
    return patches_imgs_test, patches_masks_test,test_imgs_original, test_masks, test_FOVs, test_imgs.shape[2], test_imgs.shape[3]

def get_data_test_wholeimage(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_masks, test_FOVs= load_data(test_data_path_list)
    ...
    #采用resize方式
    test_imgs_resized = np.zeros((test_imgs.shape[0], test_imgs.shape[1], 512, 512))
    test_masks_resized = np.zeros((test_masks.shape[0], test_masks.shape[1], 512, 512))
    test_FOVs_resized = np.zeros((test_FOVs.shape[0], test_FOVs.shape[1], 512, 512))
    for i in range(test_imgs_original.shape[0]):
        test_imgs_resized[i,0,:,:] = cv2.resize(test_imgs[i,0,:,:], (512, 512))
        test_masks_resized[i,0,:,:] = cv2.resize(test_masks[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)
        test_FOVs_resized[i,0,:,:] = cv2.resize(test_FOVs[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)

    # #采用crop方式
    # test_imgs_resized = test_imgs[:, :, 0:512, 0:512]
    # test_masks_resized = test_masks[:, :, 0:512, 0:512]
    # test_FOVs_resized = test_FOVs[:, :, 0:512, 0:512]

    return test_imgs_resized, test_masks_resized, test_FOVs_resized

# return tta test data
def get_data_test_overlap_tta_oneimage(test_data_path_list, pasted_vessel_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_masks, test_FOVs= load_data(test_data_path_list)
    test_imgs_original_copy = test_imgs_original.copy()
    test_imgs_original_copy = rgb2gray(test_imgs_original_copy)
    ...
    test_imgs_original_copy_resized = np.zeros((test_imgs_original_copy.shape[0], test_imgs_original_copy.shape[1], 512, 512))
    test_imgs_resized = np.zeros((test_imgs.shape[0], test_imgs.shape[1], 512, 512))
    test_masks_resized = np.zeros((test_masks.shape[0], test_masks.shape[1], 512, 512))
    test_FOVs_resized = np.zeros((test_FOVs.shape[0], test_FOVs.shape[1], 512, 512))
    for i in range(test_imgs_original.shape[0]):
        test_imgs_original_copy_resized[i,0,:,:] = cv2.resize(test_imgs_original_copy[i,0,:,:], (512, 512))
        test_imgs_resized[i,0,:,:] = cv2.resize(test_imgs[i,0,:,:], (512, 512))
        test_masks_resized[i,0,:,:] = cv2.resize(test_masks[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)
        test_FOVs_resized[i,0,:,:] = cv2.resize(test_FOVs[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)


    pasted_vessel_imgs_original,  pasted_vessel_gts, pasted_vessel_FOVs= load_data(pasted_vessel_path_list)
    pasted_vessel_imgs_original_copy = pasted_vessel_imgs_original.copy()
    pasted_vessel_imgs_original_copy = rgb2gray(pasted_vessel_imgs_original_copy)


    pasted_vessel_processed, _ = my_PreProc(pasted_vessel_imgs_original)
    pasted_vessel_gts = pasted_vessel_gts/255.
    pasted_vessel_FOVs = pasted_vessel_FOVs//255

    pasted_vessel_imgs_original_copy_resized = np.zeros((pasted_vessel_imgs_original_copy.shape[0], pasted_vessel_imgs_original_copy.shape[1], 512, 512))
    pasted_vessel_processed_resized = np.zeros((pasted_vessel_processed.shape[0], pasted_vessel_processed.shape[1], 512, 512))
    pasted_vessel_gts_resized = np.zeros((pasted_vessel_gts.shape[0], pasted_vessel_gts.shape[1], 512, 512))
    pasted_vessel_FOVs_resized = np.zeros((pasted_vessel_FOVs.shape[0], pasted_vessel_FOVs.shape[1], 512, 512))
    for i in range(pasted_vessel_imgs_original.shape[0]):
        pasted_vessel_imgs_original_copy_resized[i,0,:,:] = cv2.resize(pasted_vessel_imgs_original_copy[i,0,:,:], (512, 512))
        pasted_vessel_processed_resized[i,0,:,:] = cv2.resize(pasted_vessel_processed[i,0,:,:], (512, 512))
        pasted_vessel_gts_resized[i,0,:,:] = cv2.resize(pasted_vessel_gts[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)
        pasted_vessel_FOVs_resized[i,0,:,:] = cv2.resize(pasted_vessel_FOVs[i,0,:,:], (512, 512), interpolation=cv2.INTER_NEAREST)
    ...
    # #extract the test patches from the all test images
    # patches_imgs_test,patches_masks_test = extract_ordered_overlap(test_imgs,test_masks,patch_height,patch_width,stride_height,stride_width)
    # print("test patches shape: {}, value range ({} - {})"\
    #     .format(patches_imgs_test.shape, str(np.min(patches_imgs_test)), str(np.max(patches_imgs_test))))

    return test_imgs_original_copy_resized, test_imgs_resized, test_masks_resized, test_FOVs_resized, pasted_vessel_imgs_original_copy_resized,pasted_vessel_processed_resized, pasted_vessel_gts_resized,pasted_vessel_FOVs_resized



# extend both images and masks so they can be divided exactly by the patches dimensions
def paint_border_overlap(full_imgs, full_mask,patch_h, patch_w, stride_h, stride_w):
    ...
    if (leftover_h != 0):  #change dimension of img_h
        ...
        #todo 加入了对应的GT
        tmp_full_masks = np.zeros((full_mask.shape[0],full_mask.shape[1],img_h+(stride_h-leftover_h),img_w))
        tmp_full_masks[0:full_mask.shape[0],0:full_mask.shape[1],0:img_h,0:img_w] = full_mask
        full_mask = tmp_full_masks
    if (leftover_w != 0):   #change dimension of img_w
        ...
        full_imgs = tmp_full_imgs
        # todo 加入了对应的GT
        tmp_full_masks = np.zeros((full_mask.shape[0],full_mask.shape[1],full_mask.shape[2],img_w+(stride_w - leftover_w)))
        tmp_full_masks[0:full_mask.shape[0],0:full_mask.shape[1],0:full_mask.shape[2],0:img_w] = full_mask
        full_mask = tmp_full_masks
    print("new padded images shape: " +str(full_imgs.shape))
    return full_imgs, full_mask

# Extract test image patches in order and overlap
def extract_ordered_overlap(full_imgs,full_masks, patch_h, patch_w,stride_h,stride_w):
    assert (len(full_imgs.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3
    ...
    patches_masks = np.empty((N_patches_tot,full_masks.shape[1],patch_h,patch_w))   #todo
    iter_tot = 0   #iter over the total number of patches (N_patches)
    for i in range(full_imgs.shape[0]):  #loop over the full images
        for h in range((img_h-patch_h)//stride_h+1):
            for w in range((img_w-patch_w)//stride_w+1):
                patch = full_imgs[i,:,h*stride_h:(h*stride_h)+patch_h,w*stride_w:(w*stride_w)+patch_w]
                patch_mask = full_masks[i,:,h*stride_h:(h*stride_h)+patch_h,w*stride_w:(w*stride_w)+patch_w]  #todo
                patches[iter_tot]=patch
                patches_masks[iter_tot]=patch_mask  #todo
                iter_tot +=1   #total
