...
class Evaluate():
    ...
    # calculating f1_score_closing for closing rebuttal
    def f1_score_closing(self,threshold):
        pred = self.output>=threshold
        F1_score = f1_score(self.target, pred, labels=None, average='binary', sample_weight=None)
        # print("F1 score (F-measure): " +str(F1_score))
        return F1_score
    ...
    # Save performance results for closing rebuttal
    def save_all_result_closing(self,plot_curve=True,save_name=None, threshold=0.1):
        #Save the results

        F1_score = self.f1_score_closing(threshold)
        if save_name is not None:
            file_perf = open(join(self.save_path, save_name), 'w')
            file_perf.write("\nF1 score: " +str(F1_score)
                            )
