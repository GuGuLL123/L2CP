...
class TrainDatasetV2(Dataset):
    def __init__(self, imgs,masks,fovs,patches_idx,mode,args, closing_weight=None):
        self.imgs = imgs
    ...
    def __getitem__(self, idx):
        ...
        if self.closing_weight is not None:
            close = self.closing_weight[n,:,y_center-int(self.patch_h/2):y_center+int(self.patch_h/2),x_center-int(self.patch_w/2):x_center+int(self.patch_w/2)]
            close = torch.from_numpy(close).float()
        else:
            close = None
        ...
        if self.transforms:
            if close is not None:
                data, mask, close = self.transforms(data, mask, close, True)
            else:
                data, mask, close = self.transforms(data, mask, close, False)

        return data, mask.squeeze(0), close
...
def data_preprocess(data_path_list):
    ...
    train_imgs, closing_weight = my_PreProc(train_imgs_original)
    return train_imgs, closing_weight, train_masks, train_FOVs
